...
Enable "Notify me when the checks of a watched pull request finish" in the options to receive these notifications; it asks for the `notifications` permission. "Play a sound when the checks finish" is a separate setting from the notification sound of the count, so the checks can be heard while everything else stays quiet. The "Notification style" setting decides how the result is announced:
